...
# the two places that carry a version in step.
#
...
current=$(cargo metadata --format-version 1 --no-deps \
    | jq -r '.packages[] | select(.name == "pullkit") | .version')
...
# The window's manifest carries a version of its own; the GUI shows it.
perl -0pi -e "s/(\"version\": \")$current(\")/\${1}$next\${2}/" src-tauri/tauri.conf.json
# Bring Cargo.lock along, or the build fails on --locked with the version it still
# remembers.
cargo update --workspace --quiet

# Checked rather than assumed: a reformat would leave a substitution above
# matching nothing, and a half-bumped tree pushes a commit that releases nothing
...
if ! grep -F "\"version\": \"$next\"" src-tauri/tauri.conf.json >/dev/null; then
...
echo "  https://github.com/cyberneura/pullkit/actions/workflows/release.yml"
